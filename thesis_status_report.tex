...
\begin{titlepage}
  ...
  Improving Reliability, Control, and Training Efficiency in Large Language Models}}\\[3cm]
...
Large Language Models (LLMs) have demonstrated remarkable capabilities across diverse tasks, yet their deployment in production systems faces critical challenges: \textbf{unreliable outputs} (hallucinations, miscalibrated confidence), limited \textbf{controllability}, and \textbf{inefficient training}. This thesis investigates how to improve reliability, control, and training efficiency by leveraging signals from both \textbf{internal representations} (hidden state geometry) and \textbf{output distributions} (token-level entropy) at two stages:
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Output distributions:} Token-level entropy reflects model confidence and task complexity, enabling runtime uncertainty characterization to flag potential hallucinations (Study 1) and training-time data stratification for more effective fine-tuning (Study 3).
...
\subsubsection{Objective 1: Output Distribution Analysis}
\begin{itemize}[leftmargin=*]
...
\subsubsection{Objective 2: Latent Space Structure Exploration}
\begin{itemize}[leftmargin=*]
...
\subsubsection{Objective 3: Uncertainty-Guided Efficient Training}
\begin{itemize}[leftmargin=*]
...
\subsubsection{Objective 4: Synthesis and Thesis Completion}
\begin{itemize}[leftmargin=*]
  \item Articulate unified framework: improving reliability, control, and training efficiency via internal representations and output distributions
